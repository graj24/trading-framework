...
EARNINGS BEAT AVG REACTION: {rag.get('earnings_beat_avg', 'N/A')}%"""
        ...
        _api_base = llm_cfg.get("api_base")
        ...
        _kwargs = dict(model=_model, messages=[{"role": "user", "content": prompt}],
...
class MasterAgent(Agent):
    ...
    def run(self, context: Optional[dict] = None) -> AgentResult:
        ...
        self._pm_id = str((context or {}).get("pm_id", "1"))
    ...
    def run_for_stock(self, symbol: str) -> AgentResult:
        ...
        scores = {
            ...
            "regime_proba": regime.get("regime_proba", {}),   # Stage 3b soft proba
        }
        ...
        # Stage 4: learned_tech_score — calibrated GBM replaces the 0-10 sum.
        # Falls back to TechnicalAgent's score when model not trained.
        try:
            from models.learned_tech_score import predict_proba as lts_proba
            path = __import__("pathlib").Path("stocks") / symbol / "price_history.parquet"
            if path.exists():
                import pandas as _pd
                _df = _pd.read_parquet(path).sort_index()
                _df.index = _pd.to_datetime(_df.index, utc=True).tz_localize(None)
                lts = lts_proba(_df)
                if lts is not None:
                    # Convert calibrated proba (0-1) to 0-10 scale for backward compat.
                    scores["technical_score"] = round(lts * 10, 2)
                    scores["learned_tech_proba"] = round(lts, 4)
                    logger.info(f"{symbol}: learned_tech_score proba={lts:.3f}")
        except Exception as e:
        ...
        # If a model exists for this PM (>= 30 closed trades), surface its
        # P(trade profitable) as `meta_proba`. Downstream LLM prompt and
        # rule-based fallback can use it as a final confidence modifier.
        pm_id = str(getattr(self, "_pm_id", "1"))
        try:
            ...
            if meta_p is not None:
                logger.info(f"{symbol}: PM{pm_id} meta_proba={meta_p:.3f}")
        except Exception as e:
