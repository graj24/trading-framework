...
## Wave H — Code-review fixes (2026-05-17)

**Branch**: `feat/bloomberg-ui`  
**Tests**: **166 passing** (+18 new), 0 failing.

| ID  | File(s)                                          | What                                                                 |
|-----|--------------------------------------------------|----------------------------------------------------------------------|
| B-1 | `core/replay.py`                                 | Fixed MACD formula (`histogram > 0`) and added missing filter guard  |
| N-2 | `tests/test_n2_replay_gap_equivalence.py`        | Equivalence test: replay vs GapStrategy on same synthetic data       |
| M-3 | `ml_model.py`, `india_intraday_model.py`         | AUC floor 0.55 — rejects degenerate models even with no incumbent    |
| M-4 | `ml_model.py`, `india_intraday_model.py`         | Renames current `.pkl` → `_prev.pkl` before promoting new model      |
| M-2 | `core/duckdb_store.py`                           | `STOCKS_DIR` imported from `core.knowledge_base` (absolute path)     |
| N-4 | `core/replay.py`                                 | `REPLAY_DB` made absolute (`__file__`-relative)                      |
| M-1 | `core/duckdb_store.py`                           | `_date_where` validates ISO date format; raises `ValueError` on injection strings |
| M-5 | `core/broker.py`                                 | `ShadowBroker._fill_log` capped to last 1000 entries                 |
| N-1 | `core/replay.py`                                 | Docstring corrected: gap strategy only, not full pipeline            |
| N-3 | `core/duckdb_store.py`                           | `_index_col` decorated with `lru_cache(256)` — one connection per path |
| N-5 | `agents/execution_agent.py`                      | `signal_source` check uses `is not None` instead of falsy `or`       |
| N-6 | `ui/pages/3_Dashboard.py`                        | Added comment clarifying intentionally-thin wrapper design           |
| N-7 | `core/broker.py`                                 | `get_broker` raises `ValueError` when `broker` key missing in live mode |

All code-review findings resolved. Remaining: A1, E4 (user-only); real broker impls; full-pipeline replay; backtest re-run.

---

## Wave I — Full-pipeline replay (2026-05-17)

**Branch**: `feat/bloomberg-ui`  
**Tests**: **175 passing** (+9 new), 0 failing.

| ID  | File(s)                                    | What                                                                              |
|-----|--------------------------------------------|-----------------------------------------------------------------------------------|
| I-1 | `core/backtester.py:Strategy`              | Added `signal(pit_df, symbol) -> Trade | None` to ABC (default raises `NotImplementedError`) |
| I-2 | `core/backtester.py:GapStrategy`           | Implemented `signal()` — per-row logic extracted from `trades()`                 |
| I-3 | `core/replay.py`                           | Added `replay_strategy(strategy, symbols, start, end, db_path)` — drives any `Strategy` subclass with PIT slices |
| I-4 | `core/replay.py` CLI                       | `--strategy gap|ml_intraday` flag; dispatches to `replay_strategy()`             |
| I-5 | `tests/test_replay_strategy.py`            | 9 tests: signal() correctness, equivalence, custom strategy, empty-data guard    |

Usage: `python -m core.replay --start 2025-01-01 --end 2025-03-31 --strategy gap`
